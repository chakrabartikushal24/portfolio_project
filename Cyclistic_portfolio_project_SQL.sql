...
SELECT member_casual, ROUND(AVG(duration),2) as average_duration, ROUND(STDEV(duration),2) AS standard_deviation
FROM cyclistic_year_data
GROUP BY member_casual
...
-- To check the average duration of the rides by different months for each user type
...
-- too many starting longitue and latitudes have only a few ride count which is incresing the number of points on the map and making it difficult to analyse

-- again we will use a view in order to filter out all those groups having less than 100 count

WITH station_location AS (
SELECT member_casual, ROUND(start_lat,3) AS starting_lat, ROUND(start_lng,3) AS starting_lng, COUNT(*) AS count 
FROM cyclistic_year_data 
GROUP BY start_lat, start_lng, member_casual)
...
-- we will do the same with the ending latitude and longitudes and plot the results on a map

WITH station_location AS (
SELECT member_casual, ROUND(end_lat,3) AS ending_lat, ROUND(end_lng,3) AS ending_lng, COUNT(*) AS count 
FROM cyclistic_year_data 
GROUP BY end_lat, end_lng, member_casual)
